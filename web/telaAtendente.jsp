...
<html>
    ...
    <body>
        ...
        <c:choose>           
            <c:when test="${not empty users.email}"><!--Veio da tela do UPDATE--> 
                <a style="color: red" href="UserController?action=edit&cpf=<c:out value="${users.cpf}"/>&cd_perfilUsuario=2">Editar</a>
            </c:when>                          
            <c:otherwise> <!--Veio direto do login-->                               
                <a style="color: green" href="UserController?action=edit&cpf=<%=(String)request.getAttribute("cpf")%>&cd_perfilUsuario=2">Editar</a>
            </c:otherwise>
        </c:choose>
        <br>
